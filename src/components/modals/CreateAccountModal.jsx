...
export default function CreateAccountModal({ isOpen, onClose, onOpenLogin, className }) {
  if (!isOpen) return null;

  return createPortal(
    <div className="fixed inset-x-0 bottom-0 top-[72px] xl:top-[220px] z-50 flex items-center justify-center p-4">
      {/* Dark Backdrop Blur Overlay BEHIND the modal */}
      <div
        aria-hidden={true}
        className="absolute inset-0 bg-carbon-black/60 backdrop-blur-md animate-in fade-in duration-300 pointer-events-none"
      />

      {/* Crisp Sharp Create Account Modal Card AT THE FRONT */}
      <div
        className={cn(
