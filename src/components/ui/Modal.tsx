...
type ModalProps = {
  ...
  title?: string;
  size?: 'sm' | 'md' | 'lg';
  /** Set false for flows that must be dismissed via an explicit action. */
  closeOnBackdropClick?: boolean;
  /** Hides the corner X — pair with your own dismiss control. */
  showCloseButton?: boolean;
  /** Extra classes for the panel itself, not the backdrop. */
  className?: string;
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-ink/60 backdrop-blur-sm">
      ...
      >
        <GlassCard
          ...
          tabIndex={-1}
          className={cn('relative w-full p-8 focus:outline-none sm:p-10', SIZES[size], className)}
        >
          {showCloseButton && (
            <button
              type="button"
              ...
              className="absolute right-5 top-5 inline-flex h-9 w-9 items-center justify-center rounded-full text-forest transition-colors hover:bg-white/70 focus:outline-none focus-visible:ring-2 focus-visible:ring-olive/60"
            >
          ...
          {title && (
            <h2 id={titleId} className="mb-6 pr-10 font-display text-3xl text-ink">
              {title}
            </h2>
          )}
