...
export default function ArticleCardHorizontal({ article }) {
  const categoryLabel = 
    article?.category === 'law-updates' ? 'Law Updates' : 
    article?.category === 'law-blogs' ? 'Law Blogs' : 
    'Everyday Law';

  const categoryListUrl = `/everyday-law/${article?.category || 'everyday-law'}`;
    ...
    <GlassCard tone="light" className="flex flex-col p-5 gap-0 border border-white/90 hover-lift">
      ...
      <div className="flex flex-col">
        ...
        <div className="flex justify-between items-center mb-3 mt-1">
          ...
          <Link 
            to={categoryListUrl} 
            className="bg-white/60 border border-sage/30 px-3 py-1.5 rounded-full font-sans text-[10px] font-bold text-ink shadow-sm hover:bg-white transition-colors shrink-0"
          >
            {categoryLabel}
          </Link>
        </div>
        
        {/* Article Title Link -> Article Detail */}
        <Link to={articleUrl}>
