export default function ArticleHeader({ article }) {
  const dateString = article?.date || 'August 2026';
    ...
    <div className="mt-8 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4 glass !bg-white/60 border border-white/90 rounded-[40px] px-6 sm:px-8 py-4 shadow-sm">
      <div className="flex items-center gap-4">
        ...
        <div className="flex flex-col">
          <span className="font-sans text-[15px] sm:text-[17px] font-bold text-ink leading-tight">
            {article.author.name}
          </span>
          <span className="font-sans text-[13px] sm:text-[14px] text-ink/60 mt-0.5">
            {article.author.role}
          </span>
        </div>
      </div>
