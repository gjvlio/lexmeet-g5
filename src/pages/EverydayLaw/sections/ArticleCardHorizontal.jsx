...
export default function ArticleCardHorizontal({ article }) {
  ...
  return (
    <GlassCard tone="light" className="flex flex-col p-5 gap-0 border border-white/90">
      <div className="w-full h-[150px] shrink-0 mb-4">
      ...
      <div className="flex flex-col">
        <div className="flex justify-between items-center mb-3 mt-1">
          ...
          <span className="bg-white/60 border border-sage/30 px-3 py-1.5 rounded-full font-sans text-[10px] font-bold text-ink shadow-sm">
            {categoryLabel}
          </span>
        </div>
        
        <h3 className="font-display text-[18px] sm:text-[20px] font-bold text-ink leading-tight mb-2 mt-1 line-clamp-2">
          {article.title}
        </h3>
        ...
        <div className="mt-auto flex justify-center">
          <Link to={`/everyday-law/${article.category}/${article.slug}`}>
            <Button variant="olive" className="!h-9 !px-8 !text-[12px]">Read more</Button>
          </Link>
        </div>
      </div>
    </GlassCard>
